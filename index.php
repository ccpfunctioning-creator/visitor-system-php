...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errorMessage) {
        ...
        $recordId = $insertedRecord[0]['id'] ?? time();
...
<?php if ($successData): ?>
    <div class="beautiful-card mx-auto my-2 animate-fade-in shadow-lg" style="max-width: 520px;">
        ...
        <div class="p-4 p-md-5 bg-white d-flex flex-column align-items-center justify-content-center text-center">
            <div class="alert alert-success d-flex align-items-center gap-2 w-100 rounded-3 mb-4 fw-semibold justify-content-center small">✅ Record Registered &amp; Cloud Saved</div>
...
<?php else: ?>
        ...
        <form action="index.php" method="POST" enctype="multipart/form-data">
            ...
            <div id="inmateSection">
                ...
                <div class="row row-cols-1 row-cols-md-2 g-3 mb-4">
                    ...
                    <div><label class="form-label custom-label-style">Inmate Full Name</label><input type="text" name="inmateName" class="form-control target-field custom-input-style" placeholder="Enter Inmate Name"></div>
                    <div><label class="form-label custom-label-style">Relationship with Inmate</label><input type="text" name="relationship" class="form-control target-field custom-input-style" placeholder="e.g., Spouse, Sibling"></div>
                </div>
            </div>
    ...
    <script>
        ...
        // Dynamic Rows Controller Logic for Accompanying Visitors
        addAccBtn.addEventListener('click', function() {
            const currentRows = accompanyingWrapper.querySelectorAll('.acc-box').length;
            if (currentRows >= 6) { 
                alert("🛑 Limit Enforced: Maximum of 6 accompanying passengers."); 
                return; 
            }
        ...
        typeSelect.addEventListener('change', function() {
            if(this.value === 'Others') {
                ...
                inmateSection.querySelectorAll('input, select').forEach(el => el.removeAttribute('required'));
            } else {
